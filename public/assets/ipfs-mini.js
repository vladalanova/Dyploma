...
window.addEventListener("DOMContentLoaded", (event) => {
  document.getElementById('uploadButton').addEventListener('click', function (e) {
    ...
    reader.onload = function() {
      ...
      var fileDataStr = String.fromCharCode.apply(null, new Uint8Array(fileData));
      var boundary = createBoundary(fileDataStr); 
      var blob = new Blob([new Uint8Array(fileData)]);
      var fileReader = new FileReader();
      fileReader.onload = function() {
        var arrayBuffer = fileReader.result;
        ipfs.add(arrayBuffer, function (err, result) {
          console.log(result);
          if (!err) {
            var ipfsHash = result.hash;
